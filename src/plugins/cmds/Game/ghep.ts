...
async function drawUser(ctx: CanvasRenderingContext2D, x: number, y: number, user: UserDraw): Promise<void> {
  const size = 180;
  const r = size / 2;
  ...
  const nm = user.name || "Unknown";
  const maxWidth = 200;
  const lineHeight = 28;

  const textMetrics = ctx.measureText(nm);
  const lines = textMetrics.width > maxWidth ? wrapText(ctx, nm, maxWidth) : [nm];
...
async function makeImage(params: MakeImageParams): Promise<Buffer> {
  const { one, two, nameOne, nameTwo, tagOne, tagTwo, percent, message } = params;

  let img1: Awaited<ReturnType<typeof loadImage>> | null = null;
  let img2: Awaited<ReturnType<typeof loadImage>> | null = null;
  try {
    const [buf1, buf2] = await Promise.all([fetchFbAvatarBuffer(one), fetchFbAvatarBuffer(two)]);
    img1 = await loadImage(buf1);
    img2 = await loadImage(buf2);
  } catch {
    // fallback chữ cái trong drawUser
  }
  ...
  await drawUser(ctx, 150, 140, { name: nameOne, tag: tagOne, img: img1 });
  await drawUser(ctx, W - 150, 140, { name: nameTwo, tag: tagTwo, img: img2 });

  drawHeartCore(ctx, W / 2, 150, clampInt(percent, 0, 100), message);

  return canvas.toBuffer("image/png");
}

function getThreadParticipantIDs(threadData: { threadInfo?: { participantIDs?: unknown[] } } | null): string[] {
