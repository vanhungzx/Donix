...
async function getBio(uid: string, client: any): Promise<string> {
  ...
  try {
    ...
    console.error("Error getting bio:", error.message);
    return "Không có";
  }
}
...
async function getProfileCoverPhoto(uid: string): Promise<string> {
  try {
    ...
    const { data } = await axios.get(`https://www.facebook.com/profile.php?id=${uid}`, {
      headers: headers,
      timeout: 10000,
      maxRedirects: 5
    });
    ...
    console.log("Lỗi khi lấy cover photo:", e.message);
    return "Không có";
  }
}
...
const fbuserinfoCommand: Command = {
  ...
  async onCall(ctx: CommandOnCallContext): Promise<void> {
    ...
    try {
      const token = config.token?.EAAD6V7;
      if (!token) {
        await reply({
      ...
      if (event.messageReply) {
        ...
        if (isValidURL(args[0])) {
          try {
            id = (await (client.getUID as (url: string) => Promise<string>)(args[0])) || null;
          } catch {
            id = null;
          }
...
${updated}`;
      ...
      await reply({
        body: `LỖI: Đã xảy ra lỗi: ${e.message || "Lỗi không xác định"}`
      });
    }
  }
};
