...
export const decodeGroupInviteFromLsResp = (raw: string): GroupInviteJson => {
    ...
    const errorText = texts.find(t => /Liên kết\b.+(không còn hoạt động|hết hạn)/i.test(t));
    if (errorText) {
        result.errorText = errorText;
        result.isExpired = true;
    }

    // 2) Join explainer text
    const explainer = texts.find(t => /nếu bạn tham gia nhóm chat này|Nếu bạn tham gia nhóm chat này/i.test(t));
    if (explainer) {
        result.joinExplainerText = explainer;
    }

    // 3) Tóm tắt số thành viên
    const memberSummary = texts.find(t => /Bạn đã kết nối với \d+ người/i.test(t));
    if (memberSummary) {
    ...
    if (!result.threadTitle) {
        const titleCandidate = texts.find(t => /ChatBot|Nhóm|Group|Room|Chat\s*VN/i.test(t));
        if (titleCandidate) {
    ...
    const mainImg = urls.find(u => /scontent\./i.test(u)) || urls.find(u => /static\.xx\.fbcdn\.net/i.test(u)) || urls[0];
    if (mainImg) {
