...
// Upload theo "Business Facebook" (upload-business.facebook.com)
// Form field: upload_1024 (khác uploadFb: farr)
...
const BUSINESS_DOMAIN = "business.facebook.com";
const UPLOAD_DOMAIN = "upload-business.facebook.com";
const TOKEN_PAGE_URL = "https://business.facebook.com/latest/home?nav_ref=bm_home_redirect";
const UPLOAD_PATH = "/ajax/mercury/upload.php";
...
function checkpointError(
  res: { url?: string; requestUrl?: string; body?: string; statusCode?: number }
  ...
  e.code = "CHECKPOINT";
  e.checkpoint = true;
  e.url = d.url || `https://${BUSINESS_DOMAIN}/checkpoint/`;
...
async function getTokens(ua: string, forceRefresh = false): Promise<Tokens> {
  ...
  try {
    const html = await httpGet(TOKEN_PAGE_URL, ua, { Referer: `https://${BUSINESS_DOMAIN}/` });
    ...
    if (tokenCache) {
      logger.warn(`[uploadFb2] Token fetch failed, using cached tokens: ${(e as Error).message}`);
      return tokenCache;
    }
    throw e;
  }
...
function mapAttachmentDetails(data: unknown): AttachmentDetail[] {
  ...
  while (stack.length) {
    ...
    if (!thumbnail) {
      const m = (obj.media || obj.thumbnail || obj.thumb || obj.image_data || obj.video_data || obj.preview) as
        | Record<string, unknown>
        | undefined;
      thumbnail =
        (m?.thumbnail_src as string) || (m?.thumbnail_url as string) || (m?.src as string) || (m?.uri as string) || (m?.url as string);
    }
...
async function singleUpload(url: string, file: NormalizedFile, ua: string, tokens: Tokens, retries = 2): Promise<{ data: unknown; status: number }> {
  ...
  form.append("upload_1024", file.stream, { filename: file.filename, contentType: file.contentType });
  ...
  const headers: Record<string, string> = {
    ...
    "sec-fetch-mode": "cors",
    "sec-fetch-site": "same-origin",
    Origin: `https://${BUSINESS_DOMAIN}`,
    Referer: `https://${BUSINESS_DOMAIN}/`,
    "x-fb-rlafr": "0",
    Connection: "keep-alive",
  };
...
export default function (_defaultFuncs: unknown, _api: unknown, ctx: Context) {
  const ua =
    (ctx?.options?.userAgent as string) ||
  ...
  return function uploadFb2(link: unknown, opts?: UploadOptions | UploadCallback, callback?: UploadCallback): Promise<UploadResult> {
    ...
    (async () => {
      ...
      try {
        ...
        // Build upload URL with tokens (Business upload)
        const reqId = Math.floor(Math.random() * 36 ** 3).toString(36);
        const userId = String(ctx?.userID ?? "");
        const q = (key: string, val?: string): string => (val != null && val !== "" ? `${key}=${encodeURIComponent(String(val))}` : "");
        const queryString = [
          "__aaid=0",
          "__a=1",
          q("__req", reqId),
          ...
          q("__rev", tokens.rev),
          "__comet_req=11",
          q("fb_dtsg", tokens.fb_dtsg),
          q("jazoest", tokens.jazoest),
          q("lsd", tokens.lsd),
          q("__spin_r", tokens.spin_r),
          `__spin_b=${encodeURIComponent("trunk")}`,
          q("__spin_t", tokens.spin_t),
          "__jssesw=1",
          q("__user", userId),
          q("request_user_id", userId),
        ]
          .filter(Boolean)
          .join("&");

        const baseUrl = `https://${UPLOAD_DOMAIN}${UPLOAD_PATH}?${queryString}`;

        if (options.mode === "single") {
          const form = new FormData();
          for (const f of normAll) {
            form.append("upload_1024", f.stream, { filename: f.filename, contentType: f.contentType });
          }
          ...
          if (!ids.length) {
            const e = new Error("UploadFb2 returned no metadata/ids") as Error & {
              code?: string;
              status?: number;
              body?: unknown;
            };
          ...
          logger.info(`[uploadFb2] success ${ids.length} item(s) status ${res.status}`);
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          try {
            const cp = checkpointError({
              url:
                (res as { request?: { res?: { responseUrl?: string } }; config?: { url?: string } }).request
                  ?.res?.responseUrl || (res as { config?: { url?: string } }).config?.url,
              requestUrl:
                (res as { request?: { res?: { responseUrl?: string } }; config?: { url?: string } }).request
                  ?.res?.responseUrl || (res as { config?: { url?: string } }).config?.url,
              body:
                typeof (res as { data?: unknown }).data === "string"
                  ...
                  : String((res as { data?: unknown }).data || ""),
            ...
            if (!fileIds.length) {
              ...
              logger.warn(`[uploadFb2] File ${i + 1} returned no metadata/ids`);
              continue;
            }
            ...
            logger.error(`[uploadFb2] Upload ${i + 1} failed: ${error.message}`);
          }
        }
        ...
        logger.info(
          `[uploadFb2] success ${ids.length}/${normAll.length} item(s) via parallel uploads c=${uploadConcurrency}${errors.length > 0 ? ` (${errors.length} failed)` : ""}`
        );
        ...
        if (error.code === "CHECKPOINT" || (error.response && error.response.status && [401, 403].includes(error.response.status))) {
          ...
          try {
            ...
            logger.info(`[uploadFb2] Tokens refreshed after error`);
          } catch (refreshErr) {
            logger.error(`[uploadFb2] Token refresh failed: ${(refreshErr as Error).message}`);
          }
        }

        const level = error.code === "NO_METADATA" ? "warn" : "error";
        (logger as any)[level](
          `[uploadFb2] ${level} ${error.code || error.response?.status || ""} ${error.message || error}`
        );
        return callback(error);
      }
    })().catch((err) => {
      // Catch any unhandled errors in the async IIFE (shouldn't happen, but safety net)
      logger.error(`[uploadFb2] Unhandled promise rejection: ${(err as Error).message || err}`);
      rejectFunc(err);
    });
