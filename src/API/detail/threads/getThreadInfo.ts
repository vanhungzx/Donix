...
const Utils = {
    ...
    makeParsable: (data: string) => {
        const withoutForLoop = data.replace(/for\s*\((\s*;\s*;\s*)\s*\)\s*;\s*/, "");
        const maybeMultipleObjects = withoutForLoop.split(/\}\s*\{/);
        if (maybeMultipleObjects.length === 1) return maybeMultipleObjects[0];
        return `[${maybeMultipleObjects.join("},{")}]`;
    },
    parseFromBody: (body: string) => {
        if (typeof body !== "string") return body;
        try {
            const result = JSON.parse(Utils.makeParsable(body));
            const type = Utils.getType(result);
            return type === "Object" || type === "Array" ? result : body;
        } catch {
            return body;
        }
    }
...
function buildForm(queriesJson: string): any {
    const form: any = {
...
export default function getThreadInfoGraphQL(
    ...
    return function getThreadInfo(threadID: string, callback?: (err: any, data?: ThreadInfo) => void): Promise<ThreadInfo> {
        ...
        const form = buildForm(JSON.stringify(q));

        api.httpPost("https://www.facebook.com/api/graphqlbatch/", form)
            .then((raw: any) => {
